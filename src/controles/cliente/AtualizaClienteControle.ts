...
class AtualizaClienteControle {
    async handle(req: Request, res: Response) {
        ...
        const { nome, endereco, bairro, cidade, estado, whatsapp, dataNascimento, inscricaoEstadualRg, CEP } = req.body

        const atualizaClienteServico = new AtualizaClienteServico()
        const itemDoPedidoAtualizado = await atualizaClienteServico.execute({ nome, endereco, bairro, cidade, estado, whatsapp, dataNascimento, inscricaoEstadualRg, CEP, clienteID })
